   ...
   (try (let [content (xml/parse (str karotz-api url))  
     ...
     (if (error? code)
       ...
       (tag-content :interactiveId content)))
          (catch IOException e (str "ERROR " e)))))
...
(defn valid-id? [token]
    (let [response (karotz-request token "ears?left=10&right=-10&relative=true")]
      (not (error? response))))

(defn sign-in 
  "sign-ins to karotz with provided credentials. Data should be provided as map.
  {:api-key <api-key> :secret <secret> 
   :interactive-ids {<karotz-id> <last known interactive id>}}"  
...
(defn move-ears [token]
  (let [request-url "ears?left=20&right=-30&relative=true"]
